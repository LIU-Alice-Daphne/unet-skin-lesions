...
def parse_args():
    ...
    parser.add_argument('--dataset', default='dsb2018_96',
                        help='dataset name')
    parser.add_argument('--img_ext', default='.jpg',  # <--- 修正为 .jpg
...
def main():
    ...
    # Data loading code - 统一从 config['dataset'] 加载所有图片ID并进行划分
    all_img_ids = glob(os.path.join('inputs', config['dataset'], 'images', '*' + config['img_ext']))
    all_img_ids = [os.path.splitext(os.path.basename(p))[0] for p in all_img_ids]

    if not all_img_ids:
        print(f"错误: 未在 '{os.path.join('inputs', config['dataset'], 'images')}' 目录下找到任何 '{config['img_ext']}' 格式的图片。")
        print("请检查您的图片路径和 'img_ext' 配置，或确认该目录下有正确格式的图片。")
        exit()  # 提前退出以避免后续错误

    # 使用 train_test_split 划分训练集和验证集
    train_img_ids, val_img_ids = train_test_split(all_img_ids, test_size=0.2, random_state=41)
    ...
    for epoch in range(config['epochs']):
        ...
        print(
            'loss %.4f - iou %.4f - dice %.4f - recall %.4f - val_loss %.4f - val_iou %.4f - val_dice %.4f - val_recall %.4f'
            % (train_log['loss'], train_log['iou'], train_log['dice'], train_log['recall'], val_log['loss'],
               val_log['iou'], val_log['dice'], val_log['recall']))
        ...
        try:
            if 'loss' in globals() and callable(loss) and loss.__module__ == 'loss':
                loss.loss(config['name'])
        except AttributeError:
            print(
                "Warning: Could not call loss.loss(config['name']). Ensure 'loss' is correctly imported and has a callable 'loss' attribute.")
        except Exception as e:
            print(f"Error calling custom loss function: {e}")
        ...
        if val_log['loss'] < best_loss:
            ...
            trigger = 0
        if (val_log['loss'] < best_loss50) and (i >= 60):
